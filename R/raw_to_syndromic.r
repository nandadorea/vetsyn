...
##' Will count the number of cases for one or more defined groups, daily.
...
##' groups with zero events in the specific data chunk being analyzed will
...
##' \describe{
##'  \item{observed}{A \code{matrix} with as many columns as syndromic groups
##'  found in the dataset (or listed by the user).}
##'  \item{dates}{A \code{DataFrame} where the first column contains the complete
...
##'  the class \code{syndromic} is created. Weekends are removed if the user
##'  has employed the function setting "merge.weekend" to a number between 1 and 5.}
##'    }
##' @import ISOweek
##' @examples
##' data(lab.daily)
...
##' \dontrun{
##'  my.syndromic
##'  head(my.syndromic@observed,7)
##'  head(my.syndromic@dates,7)
##'  }
##'
